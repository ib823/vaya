...
pub fn create_pool_handler(req: &Request) -> ApiResult<Response> {
    ...
    Ok(Response::created().with_body(br#"{"pool_id":"pool_123","status":"open","created_at":"2026-01-09T00:00:00Z"}"#.to_vec()))
}
...
pub fn get_pool_handler(req: &Request) -> ApiResult<Response> {
    let _id = req.param("id").ok_or(ApiError::bad_request("Missing pool ID"))?;
    // TODO: Implement pool retrieval
    Ok(Response::ok().with_body(br#"{"pool_id":"pool_123","status":"open","members":0,"target_size":10}"#.to_vec()))
}
...
pub fn submit_bid_handler(req: &Request) -> ApiResult<Response> {
    ...
    Ok(Response::created().with_body(br#"{"bid_id":"bid_123","pool_id":"pool_123","status":"pending"}"#.to_vec()))
}
...
pub fn accept_bid_handler(req: &Request) -> ApiResult<Response> {
    ...
    Ok(Response::ok().with_body(br#"{"pool_id":"pool_123","bid_id":"bid_123","accepted":true}"#.to_vec()))
}
